// ============================================================
// NUBA INCIDENCIAS — Tab: Calendario (con importador y vista visual)
// ============================================================
import { useState, useEffect } from "react";
import { supabase } from "../../lib/supabase";
import { useAuth } from "../../lib/AuthContext";
import ImportadorICNEA from "../ImportadorICNEA";
import CalendarioVisual from "../CalendarioVisual";

export default function Calendario({ apartamento, apartamentos }) {
  const { esAdmin } = useAuth();
  const [vista, setVista]       = useState("visual"); // "visual" | "reservas" | "importar"
  ...
  useEffect(() => { if (apto && vista === "reservas") cargar(); }, [apto, vista]);
  ...
  return (
    <div className="max-w-full mx-auto">
      ...
      <div className="flex gap-1 bg-slate-100 rounded-xl p-1 w-fit mb-6 flex-wrap">
        {[
          { key: "visual",    label: "🗓️ Vista general" },
          { key: "reservas",  label: "📋 Por apartamento" },
          ...(esAdmin ? [{ key: "importar", label: "📥 Importar ICNEA" }] : []),
        ].map(t => (
          <button key={t.key} onClick={() => setVista(t.key)}
            className={`px-4 py-2 rounded-lg text-sm font-semibold transition
              ${vista === t.key ? "bg-white text-nuba-blue shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>
            {t.label}
          </button>
        ))}
      </div>

      {/* ── VISTA GENERAL (estilo ICNEA) ── */}
      {vista === "visual" && (
        <CalendarioVisual apartamentos={apartamentos} />
      )}

      {/* ── IMPORTADOR ── */}
      {vista === "importar" && esAdmin && (
        <div className="max-w-2xl">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 text-sm text-blue-700">
            <p className="font-semibold mb-1">ℹ️ Cómo exportar desde ICNEA:</p>
            <p className="font-medium mt-2">Calendario (.xhtml):</p>
            <ol className="list-decimal list-inside mt-1 space-y-0.5 text-xs">
              <li>Entra en ICNEA → Bookings → Bookings calendar</li>
              <li>Selecciona el mes que quieres exportar</li>
              <li>Haz clic en "Download" arriba a la derecha</li>
              <li>Descarga el archivo .xhtml y súbelo aquí</li>
            </ol>
            <p className="font-medium mt-2">Incidencias (.csv):</p>
            <ol className="list-decimal list-inside mt-1 space-y-0.5 text-xs">
              <li>Entra en ICNEA → Administration → Incidents and maintenance</li>
              <li>Haz clic en "Download CSV"</li>
